...
export class CheckoutPaymentComponent implements OnInit {
  ...
  ngOnInit(): void {
    loadStripe(
      'pk_test_51OUfSOLWNQUTdssy1s68Yp4rnt7arph5lrhDttqFqyayaT3NSo87V3amVJgRGV5FnNGAJDikchP0FpX9ONH7lRtc005WvuPI02'
    ).then((stripe) => {
      this.stripe = stripe;
  ...
  private async confirmPaymentWithStripe(basket: Basket | null) {
    ...
    const results = this.stripe?.confirmCardPayment(basket.clientSecret!, {
      payment_method: {
        ...
        billing_details: {
          name: this.checkoutForm?.get('paymentForm')?.get('name')?.value,
        },
      },
    });
    if (!results) throw new Error('Problem with stripe');
    return results;
  }
